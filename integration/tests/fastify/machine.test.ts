...
test.describe('Fastify machine authentication @machine', () => {
  test.describe('API key auth', () => {
    ...
    test.beforeAll(async () => {
      ...
      app = await appConfigs.fastify.vite
        .clone()
        .addFile(
          'src/server/main.ts',
          () => `
          ...
          async function start() {
            ...
            fastify.register(clerkPlugin, { publishableKey: process.env.VITE_CLERK_PUBLISHABLE_KEY });
        ...
        .addFile(
          ...
          async function start() {
            ...
            fastify.get('/m2m', async (request, reply) => {
              const { subject, tokenType, isAuthenticated } = getAuth(request, { acceptsToken: 'm2m_token' });
              if (!isAuthenticated) {
                return reply.code(401).send({ error: 'Unauthorized' });
              }
