...
testAgainstRunningApps({ withEnv: [appConfigs.envs.withReverification] })(
  '@nextjs authorization with protectAction',
  ({ app }) => {
    ...
    test('protectAction returned values as `admin`', async ({ page, context }) => {
      ...
      await u.page.goToRelative('/protect-action/signed-out');
      await u.page.getByRole('button', { name: /LogUserId/i }).click();
      await expect(u.page.getByText(/\{\s*"userId"\s*:\s*"user_[^"]+"\s*\}/i)).toBeVisible();

      await u.page.goToRelative('signed-out-with-role');
      await u.page.getByRole('button', { name: /LogUserId/i }).click();
      await expect(u.page.getByText(/\{\s*"userId"\s*:\s*"user_[^"]+"\s*\}/i)).toBeVisible();

      await u.page.goToRelative('signed-out-with-reverification');
      await u.page.getByRole('button', { name: /LogUserId/i }).click();
      await expect(u.page.getByText(/\{\s*"userId"\s*:\s*"user_[^"]+"\s*\}/i)).toBeVisible();

      await u.page.goToRelative('signed-out-with-all');
      await u.page.getByRole('button', { name: /LogUserId/i }).click();
      await expect(u.page.getByText(/\{\s*"userId"\s*:\s*"user_[^"]+"\s*\}/i)).toBeVisible();
      ...
      await page.waitForTimeout(total / 3);
      await u.page.goToRelative('/protect-action/signed-out-with-reverification');
      await u.page.getByRole('button', { name: /LogUserId/i }).click();
      await expect(
        u.page.getByText(
    ...
    test('protectAction returned values as `signed-out user`', async ({ page, context }) => {
      const u = createTestUtils({ app, page, context });

      const routes = [
        '/protect-action/signed-out',
        '/protect-action/signed-out-with-role',
        '/protect-action/signed-out-with-reverification',
        '/protect-action/signed-out-with-all',
      ];
    ...
    test('protectAction returned values as `signed-in user`', async ({ page, context }) => {
      test.setTimeout(1200000);
      const u = createTestUtils({ app, page, context });
      ...
      await expect(
        u.page.getByText(
          // /\{\s*"userId"\s*:\s*"user_[^"]+"\s*,\s*"fva"\s*:\s*.*?\s*\}/i
          /\{\s*"clerk_error"\s*:\s*\{\s*"type"\s*:\s*"forbidden"\s*,\s*"reason"\s*:\s*"assurance"\s*,\s*"metadata"\s*:\s*\{\s*"level"\s*:\s*"secondFactor"\s*,\s*"afterMinutes"\s*:\s*1\s*\}\s*\}\s*\}/i,
          // /\{\s*"userId"\s*:\s*\{\s*"type"\s*:\s*".*?"\s*,\s*"reason"\s*:\s*".*?"\s*,\s*"metadata"\s*:\s*\{\s*"role"\s*:\s*".*?"\s*\}\s*\}\s*\}/i,
        ),
      ).toBeVisible();
    });
