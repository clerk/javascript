...
testAgainstRunningApps({ withEnv: [appConfigs.envs.withReverification] })(
  '@nextjs authorization with protectRoute',
  ({ app }) => {
    ...
    test('protectRoute returned values as `admin`', async ({ page, context }) => {
      ...
      await u.page.goToRelative('/protect-route/signed-out');
      ...
      await page.waitForTimeout(total / 3);
      await u.page.goToRelative('/protect-route/signed-out-with-reverification');
      await u.page.getByRole('button', { name: /LogUserId/i }).click();
      await expect(
        u.page.getByText(
    ...
    test('protectRoute returned values as `signed-out user`', async ({ page, context }) => {
      const u = createTestUtils({ app, page, context });

      const routes = [
        '/protect-route/signed-out',
        '/protect-route/signed-out-with-role',
        '/protect-route/signed-out-with-reverification',
        '/protect-route/signed-out-with-all',
      ];
    ...
    test('protectRoute returned values as `signed-in user`', async ({ page, context }) => {
      test.setTimeout(1200000);
      const u = createTestUtils({ app, page, context });
