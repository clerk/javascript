...
Single-condition role, permission, feature, and plan checks (`has({ permission })`, etc.) and `reverification`-only checks are unchanged. Callback-form `auth.protect(has => ...)` is unaffected unless the callback itself invokes the affected shapes.
