...
const NextClientClerkProvider = <TUi extends Ui = Ui>(props: NextClerkProviderProps<TUi>) => {
  ...
  useSafeLayoutEffect(() => {
    window.__internal__onBeforeSetActive = intent => {
      /**
      ...
      return new Promise(resolve => {
        ...
        // When a user transitions from "signed in" to "signed out", we clear the `__session` cookie, then we call `__internal__onBeforeSetActive`.
        ...
        // Once `setActive` performs the navigation, `__internal__onAfterSetActive` will kick in and perform a router.refresh ensuring shared layouts will also update with the correct authentication context.
