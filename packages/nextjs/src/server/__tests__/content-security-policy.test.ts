  ...
  describe('createContentSecurityPolicyHeaders', () => {
    ...
    it('should create standard CSP headers with default directives', () => {
      ...
      expect(directives).toContainEqual(
        "connect-src 'self' https://clerk-telemetry.com https://*.clerk-telemetry.com https://api.stripe.com https://maps.googleapis.com https://img.clerk.com https://*.protect.clerk.com:* clerk.example.com",
      );
    ...
    it('should merge clerk subdomain with existing CSP values', () => {
      ...
      const directives = result.headers[0][1].split('; ');
      expect(directives).toContainEqual(
        `connect-src 'self' https://clerk-telemetry.com https://*.clerk-telemetry.com https://api.stripe.com https://maps.googleapis.com https://img.clerk.com https://*.protect.clerk.com:* clerk.example.com https://api.example.com`,
      );
      ...
      expect(directives).toContainEqual(
        `frame-src 'self' https://challenges.cloudflare.com https://*.js.stripe.com https://js.stripe.com https://hooks.stripe.com https://*.protect.clerk.com https://frames.example.com`,
      );
    });
    ...
    it('should produce a complete CSP header with all expected directives and special keywords quoted', () => {
      ...
      expect(directives).toContainEqual("form-action 'self'");
      expect(directives).toContainEqual(
        "frame-src 'self' https://challenges.cloudflare.com https://*.js.stripe.com https://js.stripe.com https://hooks.stripe.com https://*.protect.clerk.com value1 value2",
      );
      expect(directives).toContainEqual("img-src 'self' https://img.clerk.com");
      expect(directives).toContainEqual("style-src 'self' 'unsafe-inline'");
      expect(directives).toContainEqual("worker-src 'self' blob:");
