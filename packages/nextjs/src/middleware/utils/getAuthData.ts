...
export async function getAuthData(
  ...
  const signedOutState = { sessionId: null, session: null, userId: null, user: null, getToken };
  ...
  try {
    ...
    const user = loadUser ? await users.getUser(sessionClaims.sub as string) : undefined;
    const session = loadSession ? await sessions.getSession(sessionClaims.sid as string) : undefined;
