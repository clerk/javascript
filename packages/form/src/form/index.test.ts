...
describe('createForm async bookkeeping', () => {
  it('clears isValidating when a debounced field validator is superseded', async () => {
    vi.useFakeTimers();
    try {
      const form = createForm({ defaultValues: { username: '' } });
      createField(form, {
      ...
      form.setFieldValue('username', 'a');
      form.setFieldValue('username', 'b');
  ...
  it('settles the promise of a superseded debounced field validator', async () => {
    vi.useFakeTimers();
    try {
      const form = createForm({ defaultValues: { username: '' } });
      createField(form, {
      ...
      const first = form.validateField('username', 'change');
      const second = form.validateField('username', 'change');
      await vi.advanceTimersByTimeAsync(20);
      await allTasks();
  ...
  it('clears isValidating when a debounced form validator is superseded', async () => {
    vi.useFakeTimers();
    try {
      const form = createForm({
        defaultValues: { email: '' },
  ...
  it('completes handleSubmit when a debounced form validator was superseded', async () => {
    const onSubmit = vi.fn();
    const form = createForm({
      defaultValues: { email: '' },
      validators: {
  ...
  it('clears isFormValidating when a sync-slot form validator rejects', async () => {
    const form = createForm({
      defaultValues: { email: '' },
  ...
  it('does not recreate field meta when a pending validator settles after deleteField', async () => {
    vi.useFakeTimers();
    try {
      const form = createForm({ defaultValues: { friends: [{ name: 'a' }] } });
      createField(form, {
        name: 'friends[0].name',
        validators: {
          onChangeAsync: async () => {
      ...
      form.setFieldValue('friends[0].name', 'b');
      form.deleteField('friends[0].name');
      await vi.advanceTimersByTimeAsync(20);
      await allTasks();
      expect(form.state.fieldMeta['friends[0].name']).toBeUndefined();
    } finally {
      vi.useRealTimers();
    }
  });

  it('clears listener timers on reset', async () => {
    vi.useFakeTimers();
    try {
      const onChange = vi.fn();
      const form = createForm({
        defaultValues: { email: '' },
        listeners: { onChange, onChangeDebounceMs: 10 },
      });
      form.setFieldValue('email', 'a');
      form.reset();
      await vi.advanceTimersByTimeAsync(20);
