...
describe('createFieldGroup', () => {
  ...
  it('throws for a local key missing from the field map', () => {
    const form = createForm({ defaultValues: { billingStreet: '' } });
    const group = createFieldGroup({ form, fields: { street: 'billingStreet' } });
    expect(() => group.resolve('city')).toThrow('no mapping for local key "city"');
    expect(() => group.getFieldValue('city')).toThrow('no mapping for local key "city"');
    expect(() => group.setFieldValue('city', 'x')).toThrow('no mapping for local key "city"');
  });
