...
type ThirdPartyStrategyToDataMap = {
  [k in Web3Strategy | OAuthStrategy | AlternativePhoneCodeStrategy]: {
    id: Web3Provider | OAuthProvider | AlternativePhoneCodeProvider;
    iconUrl: string;
    name: string;
  };
};
...
type ThirdPartyProviderToDataMap = {
  [k in Web3Provider | OAuthProvider | AlternativePhoneCodeProvider]: {
    strategy: Web3Strategy | OAuthStrategy | AlternativePhoneCodeStrategy;
    iconUrl: string;
    name: string;
  };
};
...
const alternativePhoneCodeStrategies = ALTERNATIVE_PHONE_CODE_PROVIDERS.map(p => p.strategy);

const providerToDisplayData: ThirdPartyProviderToDataMap = fromEntries(
  [...OAUTH_PROVIDERS, ...WEB3_PROVIDERS, ...ALTERNATIVE_PHONE_CODE_PROVIDERS].map(p => {
...
export const useEnabledThirdPartyProviders = () => {
  const {
    ...
    social,
    alternativePhoneCodeFirstFactors,
  ...
  const knownAlternativePhoneCodeStrategies = alternativePhoneCodeFirstFactors.filter(s =>
    alternativePhoneCodeStrategies.includes(s),
  );
