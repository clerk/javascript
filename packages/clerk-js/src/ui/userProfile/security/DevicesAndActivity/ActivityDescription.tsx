...
export function ActivityDescription({
  ...
  const browser =
    `${browserName || ''} ${browserVersion || ''}`.trim() || 'Web browser';

  const location =
    [city || '', country || ''].filter(Boolean).join(', ').trim() || null;
  ...
  return (
    <div className='cl-activity-description'>
      <div className='cl-activity-prop cl-activity-type'>
        {title}
        {isCurrentSession && (
          <Tag color='success' className='cl-tag' size='sm'>
            This device
          </Tag>
        )}
      </div>
      {!!browser && (
        <div className='cl-activity-prop cl-activity-browser'>{browser}</div>
      )}
      {!!ipAddress && (
        <div className='cl-activity-prop cl-activity-ipAddress'>
          {ipAddress}
        </div>
      )}
      {!!location && (
        <div className='cl-activity-prop cl-activity-location'>{location}</div>
      )}
