...
function sessionChanged(prev: SessionResource, next: SessionResource): boolean {
  return (
    prev.id !== next.id || prev.updatedAt.getTime() < next.updatedAt.getTime()
  );
}
...
function resourceChanged<T extends AcceptedResource | undefined | null>(
  prev: T,
  next: T,
): boolean {
...
function stateWithMemoizedResources(
  cur: Resources,
  next: Resources,
): Resources {
  return {
...
export function memoizeListenerCallback(
  cb: ListenerCallback,
): ListenerCallback {
  let memo: Resources;
  return e => {
