...
export class User extends BaseResource implements UserResource {
  ...
  public toSnapshot(): UserJSONSnapshot {
    return {
      object: 'user',
      id: this.id,
      ...
      email_addresses: this.emailAddresses.map(ea => ea.toSnapshot()),
      phone_numbers: this.phoneNumbers.map(ph => ph.toSnapshot()),
      web3_wallets: this.web3Wallets.map(ph => ph.toSnapshot()),
      external_accounts: this.externalAccounts.map(ea => ea.toSnapshot()),
      passkeys: this.passkeys.map(passkey => passkey.toSnapshot()),
      organization_memberships: this.organizationMemberships.map(om => om.toSnapshot()),
      saml_accounts: this.samlAccounts.map(sa => sa.toSnapshot()),
      enterprise_accounts: this.enterpriseAccounts.map(ea => ea.toSnapshot()),
