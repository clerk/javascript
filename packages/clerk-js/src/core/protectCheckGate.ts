...
import type { BaseResource, Clerk } from './resources/internal';

type ProtectGatedResource = __internal_ProtectCheckModalProps['resource'];

const isProtectGated = (resource: BaseResource): resource is BaseResource & ProtectGatedResource =>
  'protectCheck' in resource;
 ...
 * the handler, when a resolution is already in flight (the modal's own reload and submit requests
 * land here too), or in no-RHC builds where the challenge script must not be loaded.
 */
export class ProtectCheckGate {
  ...
  public async resolve(clerk: Clerk, resource: BaseResource): Promise<void> {
    if (
      __BUILD_DISABLE_RHC__ ||
      !isProtectGated(resource) ||
