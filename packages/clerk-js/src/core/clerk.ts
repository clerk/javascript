  ...
  public setActive = async (params: SetActiveParams): Promise<void> => {
    ...
    try {
      ...
      if (typeof session === 'string') {
        ...
        // This is important for multisession apps where a new session may have been created server-side
        if (!session) {
          try {
            await this.client.reload();
            session = (this.client.sessions.find(x => x.id === sessionId) as SignedInSessionResource) || null;
          } catch (e) {
            debugLogger.warn('Failed to reload client when session not found', { sessionId, error: e }, 'clerk');
          }
        }
      }
