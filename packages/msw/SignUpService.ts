...
export class SignUpService {
  ...
  static createSignUpResponse(
    ...
    const signUpResponse = {
      ...
      verifications: {
        email_address: {
          attempts: unverifiedFields.includes('email_address') ? verificationAttempts : null,
          error: null,
          expire_at: unverifiedFields.includes('email_address') ? Date.now() + 600000 : null,
          next_action: unverifiedFields.includes('email_address') && verificationAttempts === 0 ? 'needs_attempt' : '',
          status: unverifiedFields.includes('email_address') ? verificationStatus : 'verified',
          strategy: 'email_code',
          supported_strategies: ['email_code'],
        },
        phone_number: {
          attempts: unverifiedFields.includes('phone_number') ? verificationAttempts : null,
          error: null,
          expire_at: unverifiedFields.includes('phone_number') ? Date.now() + 600000 : null,
          next_action: unverifiedFields.includes('phone_number') && verificationAttempts === 0 ? 'needs_attempt' : '',
          status: unverifiedFields.includes('phone_number') ? verificationStatus : null,
          strategy: 'phone_code',
