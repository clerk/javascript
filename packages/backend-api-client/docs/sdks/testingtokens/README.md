...
async function run() {
  const result = await clerkBackendApi.testingTokens.create();
...
import { testingTokensCreate } from "@clerk/backend-api-client/funcs/testingTokensCreate.js";
...
async function run() {
  const res = await testingTokensCreate(clerkBackendApi);
