...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListWaitlistEntriesRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/waitlist_entries")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.WaitlistEntries$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response);
  if (!result.ok) {
