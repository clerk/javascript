 ...
 * *container* (declares an ordered list of `steps` — inner sub-steps
 * the user walks through before the wizard advances to the next main
 * step). Containers are routed under their parent path
 * (e.g. `/configure/create-app`).
 */
export interface WizardStep<TData = unknown> {
   ...
   * leaf steps; ignored when `steps` is provided (the active inner
   * step's component is rendered instead).
   */
   ...
   */
  steps?: ReadonlyArray<WizardInnerStep<TData>>;
  /**
