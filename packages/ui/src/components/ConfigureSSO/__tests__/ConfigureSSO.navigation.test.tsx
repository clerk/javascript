...
// machine-level behaviours (defer/resolve, clamp, onStepChange) are unit-tested
// in `useWizardMachine.test.tsx`; these tests prove those behaviours hold when
// the connection state changes through the real query/revalidate path that the
// steps actually use, not just a hand-driven config rerender.
...
describe('ConfigureSSO wizard navigation (integration)', () => {
  ...
  // Contract rule 11: a card error surfaced on one step is cleared after the
  // active top-level step changes, via the wizard's onStepChange end-to-end.
  it('card errors do not leak across steps: a test-step error clears after navigating away', async () => {
    const { wrapper, fixtures } = await createFixtures(withAdminOrgUser);
    ...
    // Going back to configure changes the active top-level step; onStepChange
    // clears the card error so it does not leak into the next step.
