...
describe('UserVerificationFactorTwo', () => {
  ...
  mockWebAuthn(() => {
    it('renders the passkey card when passkey is the only second factor', async () => {
      ...
      const { findByText } = render(<UserVerificationFactorTwo />, { wrapper });
    ...
    it('verifies with a passkey at the second factor level when clicking the button', async () => {
      ...
      const { userEvent, findByText } = render(<UserVerificationFactorTwo />, { wrapper });
    ...
    it('starts with passkey ahead of the enrolled code factors', async () => {
      const { wrapper, fixtures } = await createFixtures(f => {
        f.withUser({ username: 'clerkuser' });
      });
      vi.spyOn(fixtures.session, 'startVerification').mockResolvedValue({
      ...
      expect(queryByText('Enter the code generated by your authenticator app to continue')).not.toBeInTheDocument();
    });

    it('lists the remaining second factors as alternative methods', async () => {
      const { wrapper, fixtures } = await createFixtures(f => {
        f.withUser({ username: 'clerkuser' });
      });
      vi.spyOn(fixtures.session, 'startVerification').mockResolvedValue({
      ...
      const { userEvent, findByText, getByText } = render(<UserVerificationFactorTwo />, { wrapper });

      await findByText('Use another method');
      await userEvent.click(getByText('Use another method'));
      await userEvent.click(await findByText('Use your authenticator app'));

      await findByText('Enter the code generated by your authenticator app to continue');
    });

    it('does not show an error when switching methods aborts a pending passkey request', async () => {
      const { wrapper, fixtures } = await createFixtures(f => {
        f.withUser({ username: 'clerkuser' });
      });
      vi.spyOn(fixtures.session, 'startVerification').mockResolvedValue({
      ...
      let rejectPasskey: (err: unknown) => void = () => {};
      vi.spyOn(fixtures.session, 'verifyWithPasskey').mockReturnValue(
        new Promise((_, reject) => {
          rejectPasskey = reject;
        }),
      );

      const { userEvent, findByText, getByText, getByRole, queryByText } = render(<UserVerificationFactorTwo />, {
        wrapper,
      });

      await userEvent.click(await findByText('Use your passkey'));
      await userEvent.click(getByText('Use another method'));
      const authenticatorAppButton = getByRole('button', { name: /Use your authenticator app/i });
      expect(authenticatorAppButton).toBeDisabled();

      rejectPasskey(new ClerkWebAuthnError('The operation was aborted.', { code: 'passkey_operation_aborted' }));

      await waitFor(() => expect(authenticatorAppButton).toBeEnabled());
      expect(queryByText(/The operation was aborted/)).not.toBeInTheDocument();
    });
  });
