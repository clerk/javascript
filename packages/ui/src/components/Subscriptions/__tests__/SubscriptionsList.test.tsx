...
describe('SubscriptionsList', () => {
  ...
  it('renders overview when the subscription has a next payment', async () => {
    const { wrapper, fixtures } = await createFixtures(f => {
      f.withUser({ email_addresses: ['test@clerk.com'] });
      f.withBilling();
    });
    ...
    fixtures.clerk.billing.getSubscription.mockResolvedValue({
      ...
      createdAt: new Date('2021-01-01'),
      nextPayment: {
        amount: { amount: 15900, amountFormatted: '159.00', currencySymbol: '$', currency: 'USD' },
    ...
    await waitFor(() => {
      ...
      expect(getByText('Billed monthly')).toBeVisible();
      expect(getByText('$159.00')).toBeVisible();
      expect(getByText('Renews Feb 1, 2021')).toBeVisible();
    });
  });
