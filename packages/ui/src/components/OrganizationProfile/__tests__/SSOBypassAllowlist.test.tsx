...
describe('SSO bypass allowlist', () => {
  ...
  describe('allow list page', () => {
    ...
    it('adds a member by email address', async () => {
      ...
      expect(await screen.findByText('Added member successfully')).toBeInTheDocument();

      await userEvent.click(await screen.findByRole('button', { name: 'Finish' }));
      await waitFor(() => expect(screen.queryByRole('heading', { name: 'Add members' })).not.toBeInTheDocument());
    });

    it('keeps looking past the first page of search results for the email address', async () => {
      const { wrapper, fixtures } = await createFixtures(
        withSecurityPage({ permissions: ['org:sys_entconns:manage', 'org:sys_entconns_sso_bypass:manage'] }),
      );
      const lookalikes = Array.from({ length: 10 }, (_, i) =>
        membership(`user_${i + 20}`, `Near${i}`, `near${i}yukio@clerk.com`),
      );
      fixtures.clerk.organization?.getMemberships
        .mockResolvedValueOnce({ data: lookalikes, total_count: 11 } as any)
      ...
      await userEvent.click(screen.getByRole('button', { name: 'Add' }));
      await userEvent.type(await screen.findByLabelText('Email address'), 'yukio@clerk.com');
      await userEvent.click(screen.getByRole('button', { name: 'Add members' }));

      await waitFor(() =>
        expect(fixtures.clerk.organization?.getMemberships).toHaveBeenCalledWith(
          expect.objectContaining({ query: 'yukio@clerk.com', initialPage: 2 }),
        ),
      );
      await waitFor(() =>
        expect(fixtures.clerk.organization?.ssoBypassAllowlist.addUser).toHaveBeenCalledWith({ userId: 'user_9' }),
    ...
    it('adds every member with the selected role and reports the skipped ones', async () => {
      ...
      expect(screen.getByText('This list does not sync, members are managed manually.')).toBeInTheDocument();
      expect(screen.getByRole('button', { name: 'Add members' })).toBeDisabled();
      ...
      expect(await screen.findByText('Added 1 member')).toBeInTheDocument();
      expect(
    ...
    it('keeps the form open when no member with the role could be added', async () => {
      const { wrapper, fixtures } = await createFixtures(
        withSecurityPage({
          permissions: ['org:sys_entconns:manage', 'org:sys_entconns_sso_bypass:manage', 'org:sys_memberships:read'],
        }),
      ...
      fixtures.clerk.organization?.getMemberships.mockResolvedValue({
        data: [membership('user_9', 'Yukio', 'yukio@clerk.com'), membership('user_10', 'Dana', 'dana@personal.com')],
        total_count: 2,
      } as any);
      fixtures.clerk.organization?.ssoBypassAllowlist.addUsers.mockResolvedValue({
        data: [],
        errors: [
          { userId: 'user_9', code: 'sso_bypass_domain_not_served' },
          { userId: 'user_10', code: 'sso_bypass_domain_not_served' },
        ],
      });
      ...
      await userEvent.click(screen.getByRole('button', { name: /select role/i }));
      await userEvent.click(await screen.findByText('Admin (2)'));
      await userEvent.click(screen.getByRole('button', { name: 'Add members' }));

      expect(
        await screen.findByText(
          '2 members could not be added because their email address is not served by a connection.',
        ),
      ).toBeInTheDocument();
    ...
    it('keeps the form open when everyone with the role is already on the list', async () => {
      const { wrapper, fixtures } = await createFixtures(
        withSecurityPage({
          permissions: ['org:sys_entconns:manage', 'org:sys_entconns_sso_bypass:manage', 'org:sys_memberships:read'],
        }),
      ...
      fixtures.clerk.organization?.getMemberships.mockResolvedValue({
        data: [membership('user_1', 'Cameron', 'cameron@clerk.com')],
        total_count: 1,
      } as any);
      ...
      await userEvent.click(await screen.findByRole('radio', { name: 'Role' }));
      await userEvent.click(screen.getByRole('button', { name: /select role/i }));
      await userEvent.click(await screen.findByText('Admin (1)'));
      await userEvent.click(screen.getByRole('button', { name: 'Add members' }));

      expect(await screen.findByText('Everyone with that role is already on the allow list.')).toBeInTheDocument();
      expect(fixtures.clerk.organization?.ssoBypassAllowlist.addUsers).not.toHaveBeenCalled();
    });
