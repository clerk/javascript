...
import { mosaicQueryClient } from './query-client';
 ...
 *
 * The module client is passed explicitly (react-query's optional `queryClient` arg) so a section
 * renders standalone — no `QueryClientProvider` needed in the tree (e.g. a per-component story).
 */
  ...
  return useMutation(
    {
      ...
      onSuccess: () => {
        ...
        void mosaicQueryClient.invalidateQueries({ queryKey: membersKey(organization.id) });
      },
    },
    mosaicQueryClient,
  );
