...
describe('trigger flags', () => {
  test('both are booleans', () => {
    accept({ renderTriggerLabel: false, renderTriggerBadge: false });
  });
  ...
  test('a truthy value of another type is rejected', () => {
    ...
    // @ts-expect-error — the badge names the active organization's plan; it is not yours to supply
    accept({ renderTriggerBadge: () => ({ name: 'Enterprise' }) });
  });
});
