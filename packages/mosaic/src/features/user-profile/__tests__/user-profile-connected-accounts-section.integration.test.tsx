...
type Reverify = 'pass' | 'cancel' | 'hint';
...
describe('UserProfileConnectedAccountsSection', () => {
  ...
  it('settles quietly when reverification is cancelled', async () => {
    reverify = 'cancel';
    const user = userEvent.setup();
    renderSection();
  ...
  it('treats a second reverification challenge as a failure, not a removal', async () => {
    const google = externalAccount({ id: 'idn_google', provider: 'google' });
    externalAccounts = [google];
    const user = userEvent.setup();
    renderSection();
