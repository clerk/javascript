...
- `status` marks the entry's maturity — `'stable'` or `'wip'` — rendered as a colored dot (green / amber) to the left of the entry in the sidebar, and as a dot-plus-label badge to the right of the page's `# Title` h1. It is explicit: omitting it displays no status anywhere (not every entry carries one, e.g. `Scroll Area`).
