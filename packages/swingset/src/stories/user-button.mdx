...
ids it knows are `switchAccount`, `addAccount`, `signOutAll`, and each custom item's `id`. Those are
the only actions the foot lists as rows; the rest live in the header, behind a `⋯`, or with the
workspaces, where there is no list for an order to run in.

The accounts row is `switchAccount` where there is somebody to switch to and `addAccount` where there
is not. Name both to place that row whichever way it resolves. An id the surface has not got is
ignored rather than an error, so one order can cover every mode.
