...
`Tabs.Indicator` is positioned against `Tabs.List` and exposes the active tab's geometry as CSS
variables:
...
to a later tab, `-1` to an earlier one) for directional panel transitions.
