...
### Managing the addresses in a row

The row's action opens a `prompt` dialog holding the same list, one `Item` per address with its own
menu. Both paths out of it stack over that dialog rather than replacing it, so the list stays on
screen behind them: adding opens a second `prompt` with its field focused
(`section → prompt → prompt`), and removing raises an `AlertDialog` to confirm
(`section → prompt → alert`).
