...
A floating list of actions opened from a trigger, from `@clerk/mosaic/primitives`. It is a
...
| <code>sideOffset</code>         | <code>SideOffset</code>              | <code>4</code>              | Gap in px between the trigger and the popup (`0` when nested)   |
| <code>fallbackPlacements</code> | <code>Placement[]</code>             | opposite side               | Where the popup goes when `placement` does not fit, in order    |

`SideOffset` is `number | { x: number; y: number }`. One number covers every placement;
`{ x, y }` gives the horizontal and vertical placements a gap each, which a popup that can flip
between the two axes wants. `fallbackPlacements` is for a menu opened from inside another floating
surface: the opposite side is that surface, so it has to be given somewhere else to land.
