...
`label` drives typeahead and is used as the visible text when `children` is omitted — enough for a
plain text row. Compose anything more from `Menu.Media` and `Menu.Label`, which lay the row out; a
bare icon and text as children do not line up across rows and do not truncate. Use
...
```tsx
<Menu.Item label='Revoke' color='negative' />
