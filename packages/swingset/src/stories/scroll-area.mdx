...
`scrollAreaViewport()` returns an array of atoms. A Mosaic part takes it as `xstyle`; a native element
...
| ------------------------------------ | --------------------- | --------------------------------------------------------------------------------- |
| `scrollAreaViewport(gutter?, axis?)` | the scrolling element | The scroll box itself: overflow, the edge fades, the scrollbar, and a focus ring. |
| `scrollAreaRoot`                     | a positioned ancestor | Only needed when something anchors against the scroll box.                        |

`gutter` is `auto` (the default) or `stable`, see [Gutter](#gutter). `axis` is `block` (the default)
or `inline`, see [Horizontal](#horizontal). Both are author-time decisions rather than theme ones:
whether a region needs a gutter depends on whether its content can resize in place, and which way it
scrolls is a property of the content itself.
...
`axis: 'inline'` turns the whole surface sideways: the box scrolls horizontally, the fades move to
the left and right edges, the scroll timeline reads inline progress, and the scrollbar lane along
the bottom is held clear of the mask. The other axis is clipped. It is the shape a wide
[Table](/components/table) takes on a narrow screen.

```tsx
<div {...stylex.props(...scrollAreaViewport('auto', 'inline'))}>{columns}</div>
```
