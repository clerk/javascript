...
A one-time-password / PIN input from `@clerk/mosaic/primitives`. It is a **headless** primitive: it owns the
...
| `disabled`      | `boolean`                                | `false`      | Disable every slot and the hidden input                |
| `required`      | `boolean`                                | `false`      | Require every slot to be filled before submitting      |
| `id`            | `string`                                 | generated    | Applied to the first slot so a `<label>` can target it |
...
Because a group is not labelable, `Otp.Root`'s `id` lands on the _first_ slot (the rest take
`${id}-2`, `${id}-3`, …), so a `<label htmlFor>` targets a real control and clicking it focuses the
code. That first slot inherits the group's name rather than a positional one — a `"Character 1 of N"`
there would contradict the visible label — while the remaining slots get a default `aria-label` of
`"Character N of M"` (overridable). The slots use a roving tab index so `Tab` enters the group once
and leaves in one step. `required` is applied to each slot, not the hidden input, which is `readOnly`
and therefore barred from constraint validation. When `name` is set, the hidden form input is
`aria-hidden`, out of the tab order, and disabled alongside the field. Arrow keys follow reading
order — under `dir="rtl"`, `ArrowLeft`/`ArrowRight` are swapped.
