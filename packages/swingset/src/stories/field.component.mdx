...
Wrap `Field.Error` in `Field.Message` and pass the message as its children. `Field.Message` measures whichever message is showing and animates its own height to match, so it grows when a message arrives, follows the text when the message changes, and collapses after it clears. A mask softens the clipped edge while the height moves and clears once it settles. The caller never conditionally mounts either part. Under `prefers-reduced-motion` the height snaps and only the message fades.
