import { parsePublishableOptions } from '@clerk/shared/keys';
import type { ClerkOptions, JwtPayload } from '@clerk/types';
import { Token } from 'src';

import { constants } from '../constants';
import { assertValidSecretKey } from '../util/assertValidSecretKey';
import type { buildRequest } from '../util/IsomorphicRequest';
import { stripAuthorizationHeader } from '../util/IsomorphicRequest';
...
import { AuthErrorReason, handshake, interstitial, signedOut, unknownState } from './authStatus';
...
export async function signedIn(
  options: AuthenticateRequestOptions & ClerkOptions,
  token: string,
...
export async function baseAuthenticateRequest(
  ...
  const { environmentType, frontendApi } = parsePublishableOptions(opts);
  ...
  assertValidSecretKey(opts.secretKey);

  // if (opts.isSatellite) {
  //   assertSignInUrlExists(opts.signInUrl, opts.secretKey);
  //   if (opts.signInUrl && opts.origin) {
  //     assertSignInUrlFormatAndOrigin(opts.signInUrl, opts.origin);
  //   }
  //   assertProxyUrlOrDomain(opts.proxyUrl || opts.domain);
  // }

  const headerToken = request.headers.get('Authorization')?.replace('Bearer ', '');
  if (headerToken) {
    const verifyResult = await verifyToken(headerToken, opts);
    if (verifyResult) {
      return signedIn(opts, verifyResult, new Headers());
    }
  }
