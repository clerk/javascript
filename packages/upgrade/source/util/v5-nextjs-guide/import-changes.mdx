...
As part of this release, we changed some of the top-level exports of `@clerk/nextjs` in order to improve bundle size and tree-shaking efficiency. These changes have resulted in a ~75% reduction in build size for middleware bundles. However, you will likely need to make some changes to import paths as a result.

<Callout type='info'>
  We recommend using our CLI tool, `npx @clerk/upgrade`, to automatically find occurences of old imports.
</Callout>
