...
Nullable and optional values remain distinct. Native `Field<T>` carries omitted, null, or value when all three states are legal. String enums retain unrecognized output values; input codecs reject values outside the declared contract. Unions carry a generated case tag and typed payload. Unknown ordinary values fail generation; only explicitly identified metadata and claims use JSON.
