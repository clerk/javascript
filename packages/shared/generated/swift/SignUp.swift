// Generated by packages/shared/scripts/generate-swift-models.ts. Do not edit.

import Foundation

public struct SignUp: Codable, Equatable, Sendable, Identifiable {
  public var object: String
  public var status: SignUpStatus
  public var requiredFields: [String]
  public var optionalFields: [String]
  public var missingFields: [String]
  public var unverifiedFields: [String]
  public var username: String?
  public var firstName: String?
  public var lastName: String?
  public var emailAddress: String?
  public var phoneNumber: String?
  public var web3Wallet: String?
  public var externalAccountStrategy: String?
  public var externalAccount: JSONValue
  public var hasPassword: Bool
  public var unsafeMetadata: JSONValue
  public var createdSessionId: String?
  public var createdUserId: String?
  public var abandonAt: Date?
  public var legalAcceptedAt: Date?
  public var locale: String?
  public var verifications: SignUpVerifications?
  public var protectCheck: ProtectCheck?
  public var id: String

  public init(
    object: String,
    status: SignUpStatus,
    requiredFields: [String],
    optionalFields: [String],
    missingFields: [String],
    unverifiedFields: [String],
    username: String? = nil,
    firstName: String? = nil,
    lastName: String? = nil,
    emailAddress: String? = nil,
    phoneNumber: String? = nil,
    web3Wallet: String? = nil,
    externalAccountStrategy: String? = nil,
    externalAccount: JSONValue,
    hasPassword: Bool,
    unsafeMetadata: JSONValue,
    createdSessionId: String? = nil,
    createdUserId: String? = nil,
    abandonAt: Date? = nil,
    legalAcceptedAt: Date? = nil,
    locale: String? = nil,
    verifications: SignUpVerifications? = nil,
    protectCheck: ProtectCheck? = nil,
    id: String
  ) {
    self.object = object
    self.status = status
    self.requiredFields = requiredFields
    self.optionalFields = optionalFields
    self.missingFields = missingFields
    self.unverifiedFields = unverifiedFields
    self.username = username
    self.firstName = firstName
    self.lastName = lastName
    self.emailAddress = emailAddress
    self.phoneNumber = phoneNumber
    self.web3Wallet = web3Wallet
    self.externalAccountStrategy = externalAccountStrategy
    self.externalAccount = externalAccount
    self.hasPassword = hasPassword
    self.unsafeMetadata = unsafeMetadata
    self.createdSessionId = createdSessionId
    self.createdUserId = createdUserId
    self.abandonAt = abandonAt
    self.legalAcceptedAt = legalAcceptedAt
    self.locale = locale
    self.verifications = verifications
    self.protectCheck = protectCheck
    self.id = id
  }

  public enum CodingKeys: String, CodingKey {
    case object
    case status
    case requiredFields = "required_fields"
    case optionalFields = "optional_fields"
    case missingFields = "missing_fields"
    case unverifiedFields = "unverified_fields"
    case username
    case firstName = "first_name"
    case lastName = "last_name"
    case emailAddress = "email_address"
    case phoneNumber = "phone_number"
    case web3Wallet = "web3_wallet"
    case externalAccountStrategy = "external_account_strategy"
    case externalAccount = "external_account"
    case hasPassword = "has_password"
    case unsafeMetadata = "unsafe_metadata"
    case createdSessionId = "created_session_id"
    case createdUserId = "created_user_id"
    case abandonAt = "abandon_at"
    case legalAcceptedAt = "legal_accepted_at"
    case locale
    case verifications
    case protectCheck = "protect_check"
    case id
  }

  public init(from decoder: Decoder) throws {
    let container = try decoder.container(keyedBy: FAPIJSONKey.self)
    self.object = try container.decodeFlexibleDefault(String.self, snake: "object", camel: "object", default: "sign_up")
    self.status = try container.decodeFlexibleDefault(SignUpStatus.self, snake: "status", camel: "status", default: .unknown(""))
    self.requiredFields = try container.decodeFlexibleDefault([String].self, snake: "required_fields", camel: "requiredFields", default: [])
    self.optionalFields = try container.decodeFlexibleDefault([String].self, snake: "optional_fields", camel: "optionalFields", default: [])
    self.missingFields = try container.decodeFlexibleDefault([String].self, snake: "missing_fields", camel: "missingFields", default: [])
    self.unverifiedFields = try container.decodeFlexibleDefault([String].self, snake: "unverified_fields", camel: "unverifiedFields", default: [])
    self.username = try container.decodeIfPresentFlexible(String.self, snake: "username", camel: "username")
    self.firstName = try container.decodeIfPresentFlexible(String.self, snake: "first_name", camel: "firstName")
    self.lastName = try container.decodeIfPresentFlexible(String.self, snake: "last_name", camel: "lastName")
    self.emailAddress = try container.decodeIfPresentFlexible(String.self, snake: "email_address", camel: "emailAddress")
    self.phoneNumber = try container.decodeIfPresentFlexible(String.self, snake: "phone_number", camel: "phoneNumber")
    self.web3Wallet = try container.decodeIfPresentFlexible(String.self, snake: "web3_wallet", camel: "web3Wallet")
    self.externalAccountStrategy = try container.decodeIfPresentFlexible(String.self, snake: "external_account_strategy", camel: "externalAccountStrategy")
    self.externalAccount = try container.decodeFlexibleDefault(JSONValue.self, snake: "external_account", camel: "externalAccount", default: .object([:]))
    self.hasPassword = try container.decodeFlexibleDefault(Bool.self, snake: "has_password", camel: "hasPassword", default: false)
    self.unsafeMetadata = try container.decodeFlexibleDefault(JSONValue.self, snake: "unsafe_metadata", camel: "unsafeMetadata", default: .object([:]))
    self.createdSessionId = try container.decodeIfPresentFlexible(String.self, snake: "created_session_id", camel: "createdSessionId")
    self.createdUserId = try container.decodeIfPresentFlexible(String.self, snake: "created_user_id", camel: "createdUserId")
    self.abandonAt = try container.decodeIfPresentMillisecondsDate(snake: "abandon_at", camel: "abandonAt")
    self.legalAcceptedAt = try container.decodeIfPresentMillisecondsDate(snake: "legal_accepted_at", camel: "legalAcceptedAt")
    self.locale = try container.decodeIfPresentFlexible(String.self, snake: "locale", camel: "locale")
    self.verifications = try container.decodeIfPresentFlexible(SignUpVerifications.self, snake: "verifications", camel: "verifications")
    self.protectCheck = try container.decodeIfPresentFlexible(ProtectCheck.self, snake: "protect_check", camel: "protectCheck")
    self.id = try container.decodeFlexible(String.self, snake: "id", camel: "id")
  }

  public func encode(to encoder: Encoder) throws {
    var container = encoder.container(keyedBy: CodingKeys.self)
    try container.encode(object, forKey: .object)
    try container.encode(status, forKey: .status)
    try container.encode(requiredFields, forKey: .requiredFields)
    try container.encode(optionalFields, forKey: .optionalFields)
    try container.encode(missingFields, forKey: .missingFields)
    try container.encode(unverifiedFields, forKey: .unverifiedFields)
    try container.encodeIfPresent(username, forKey: .username)
    try container.encodeIfPresent(firstName, forKey: .firstName)
    try container.encodeIfPresent(lastName, forKey: .lastName)
    try container.encodeIfPresent(emailAddress, forKey: .emailAddress)
    try container.encodeIfPresent(phoneNumber, forKey: .phoneNumber)
    try container.encodeIfPresent(web3Wallet, forKey: .web3Wallet)
    try container.encodeIfPresent(externalAccountStrategy, forKey: .externalAccountStrategy)
    try container.encode(externalAccount, forKey: .externalAccount)
    try container.encode(hasPassword, forKey: .hasPassword)
    try container.encode(unsafeMetadata, forKey: .unsafeMetadata)
    try container.encodeIfPresent(createdSessionId, forKey: .createdSessionId)
    try container.encodeIfPresent(createdUserId, forKey: .createdUserId)
    try container.encodeIfPresentMillisecondsDate(abandonAt, forKey: .abandonAt)
    try container.encodeIfPresentMillisecondsDate(legalAcceptedAt, forKey: .legalAcceptedAt)
    try container.encodeIfPresent(locale, forKey: .locale)
    try container.encodeIfPresent(verifications, forKey: .verifications)
    try container.encodeIfPresent(protectCheck, forKey: .protectCheck)
    try container.encode(id, forKey: .id)
  }
}
