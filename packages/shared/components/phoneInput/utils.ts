...
export function getFlagEmojiFromCountryIso(
  iso: string,
  fallbackIso = 'us',
): string {
  iso = iso || fallbackIso;
...
export function formatPhoneNumber(
  ...
  const digits = [...extractDigits(phoneNumber)].slice(
    0,
    maxE164CompliantLength(countryCode),
  );
...
function getCountryIsoFromPhoneCode(
...
function phoneNumberBelongsTo(iso: 'us' | 'ca', phoneWithCode: string) {
  ...
  return (
    ...
    phoneWithCode.length - 1 ===
      maxDigitCountForPattern(IsoToCountryMap.get(iso)?.pattern || '') &&
...
function maxE164CompliantLength(countryCode?: string) {
  ...
  const codeWithPrefix = countryCode.includes('+')
    ? countryCode
    : '+' + countryCode;
