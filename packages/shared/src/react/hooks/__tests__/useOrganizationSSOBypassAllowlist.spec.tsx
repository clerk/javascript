...
describe('useOrganizationSSOBypassAllowlist', () => {
  ...
  it('refetches the list even when a bulk add fails part way', async () => {
    addManySpy.mockRejectedValueOnce(new Error('network'));
    const { result } = renderAllowlist();
    await waitFor(() => expect(result.current.isLoading).toBe(false));

    await expect(result.current.addUsers({ userIds: ['user_2', 'user_3'] })).rejects.toThrow('network');
