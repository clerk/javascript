...
describe('retry', () => {
  ...
  test('respects firstDelay option', async () => {
    ...
    retry(
      ...
      { firstDelay: 200, jitter: false, shouldRetry: (_, count) => count <= 2 },
    ).catch(() => {});
  ...
  test('respects retryImmediatelyOnce option', async () => {
    let attempts = 0;
    retry(
      () => {
        attempts++;
