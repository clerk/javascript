...
Pod::Spec.new do |s|
  ...
  # ClerkViewFactory.swift (with views) is injected into the app target by the
  # config plugin because it uses `import ClerkKit`, which is only available via
  # SPM in the app target.
