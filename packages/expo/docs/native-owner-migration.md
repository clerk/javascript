...
The iOS bridge uses Swift 6 and iOS 17 or newer. The Android config plugin defaults to Kotlin 2.3.20 when the app has no explicit Kotlin version. Expo SDK 57's native module compiles with that compiler against the SDK built with Kotlin 2.4.10. Configure custom Kotlin/Compose versions consistently; Expo's optional compiler plugins may not support newer compilers immediately. The plugin no longer suppresses incompatible Kotlin metadata checks. The plugin configures R8 9.1.43 in Android settings so D8/R8 can process the native SDK's Kotlin 2.4 metadata. Set the config-plugin option `androidR8Version` to a tested compatible version for a custom toolchain, or `false` to use the app's compiler. Disabling the override requires an AGP/R8 combination that supports Kotlin 2.4; changing the Kotlin source compiler alone does not change metadata in an already compiled dependency. Client credentials remain in the Expo token cache. Native secure storage is used only for magic-link and biometric platform state.
...
The [Android compiler build evidence](android-compiler-build.md) records the Kotlin-metadata compatibility correction, matching debug/release artifacts, and remaining UI verification boundary.
