...
export async function createHostedAuth(
  ...
  if (
    ...
    // The Electron response hook has already persisted the replacement device token.
  ...
  if (!hostedAuthJSON?.url) {
    throw new Error('Clerk: hosted auth creation returned an invalid response.');
  }
...
export async function redeemHostedAuth(
  ...
  if (!clientJSON) {
    throw new Error('Clerk: hosted auth completion returned an invalid response.');
  }
...
export function applyHostedAuthClientJSON(client: ClientResource, clientJSON: ClientJSON): ClientResource {
  ...
  if (typeof mutableClient.fromJSON !== 'function') {
    throw new Error('Clerk: hosted auth completion could not update the current client.');
  }
