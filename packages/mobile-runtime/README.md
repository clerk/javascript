...
A separately fetched resource can share the active root's server ID while remaining a distinct JavaScript instance. Such a returned resource receives its own handle: root reconciliation must not overwrite its fresh fields with the older root's fields. Returning the exact root instance still preserves the root handle. Collection refreshes retain the existing non-root identity policy.
...
Standalone initialization includes the native package's `sdkVersion`, used for the matching `x-ios-sdk-version` or `x-android-sdk-version` header. A low-level host that omits the version sends no invented version header. Malformed version values fail initialization before HTTP. The Apple host also supplies `isMobile: false` on macOS and Mac Catalyst, and `true` elsewhere. TypeScript maps that fact to `x-mobile: 0` or `1`; omission defaults to `1` for existing mobile hosts, and non-boolean values fail before HTTP. Both classes retain `_is_native=1` and native credential handling. The SDK platform remains `ios` across Apple targets because it identifies the SDK family.
...
Generated session authorization is exercised by `test/session-authorization.test.mjs` using actual resource handles and JWT claims. It covers local role, permission, feature, plan, and reverification decisions plus revoked membership permissions after `Session.reload`. The public source union accepts one authorization criterion with optional reverification. Unsupported input levels fail bridge validation; omitted or malformed required factor-age pairs fail resource projection, while explicit null denies reverification. Native migration audits record the differences from the previous mutable native `Session` helper.

Thrown `ClerkAPIError` instances from verification resources use the same structured error details as HTTP response errors. The bridge preserves their code, message, long message and allowed metadata without inventing an HTTP status. Unknown rejection messages stay generic. `mobile-sso.test.mjs` exercises this through Apple account transfer and verifies restriction fallback boundaries.
