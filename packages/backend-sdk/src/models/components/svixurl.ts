...
/** @internal */
export const SvixURL$inboundSchema: z.ZodType<SvixURL, z.ZodTypeDef, unknown> = z
  .object({
    svix_url: z.string(),
  })
  .transform(v => {
...
/** @internal */
export const SvixURL$outboundSchema: z.ZodType<SvixURL$Outbound, z.ZodTypeDef, SvixURL> = z
  .object({
    svixUrl: z.string(),
  })
  .transform(v => {
...
export function svixURLFromJSON(jsonString: string): SafeParseResult<SvixURL, SDKValidationError> {
  return safeParse(jsonString, x => SvixURL$inboundSchema.parse(JSON.parse(x)), `Failed to parse 'SvixURL' from JSON`);
}
