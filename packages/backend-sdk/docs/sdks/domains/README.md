...
async function run() {
  const result = await clerk.domains.list();
...
async function run() {
  const res = await domainsList(clerk);
...
**Promise\<[components.Domains](../../models/components/domains.md)\>**
...
async function run() {
  const result = await clerk.domains.add();
...
async function run() {
  const res = await domainsAdd(clerk);
