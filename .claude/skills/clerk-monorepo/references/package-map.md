...
- **A React hook**: `useAuth` is implemented in `packages/react/src`; most others (`useUser`,
  ...
  re-exporting it).
- **Mosaic components (`UserButton`, StyleX parts, flow views)**: `packages/mosaic/src` (`@clerk/mosaic`). Preview in Swingset (`pnpm dev:swingset`). Headless primitives live in `packages/mosaic/src/primitives`. Mosaic-only changesets should not list `@clerk/react` or `@clerk/nextjs`.
