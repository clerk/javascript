...
| Bumping clerk-ios / clerk-android                  | clerk-ios bumps are automated by a PR bot; clerk-android is hand-pinned in `android/build.gradle`. See "Native SDK version pinning" in [`references/native-architecture.md`](references/native-architecture.md) before touching either.                                                   |
